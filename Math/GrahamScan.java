...
public static Point[] grahamScan(Point[] points) {
	...
	for(int j = s.size()-1; j >= 0; j--)
		convexHull[j] = s.pop();
	return convexHull;
	/*Sometimes it might be necessary to also add points to the convex hull that form a straight angle. The following lines of code achieve this. Only at the first and last diagonal we have to add those. Of course the previous return-statement has to be deleted as well as allowing straight angles in the above implementation. */
}
//END
...
class Point implements Comparable<Point> {
	...
	//if same angle, sort by dist
	if(Calc.crossProd(a, b) == 0 && Calc.dotProd(a, b) > 0)
		return Integer.compare(Calc.dotProd(a, a), Calc.dotProd(b, b));
	//angle of a is 0, thus b>a
	if(a.y == 0 && a.x > 0) return -1;
	//angle of b is 0, thus a>b
	if(b.y == 0 && b.x > 0) return 1;
	//a ist between 0 and 180, b between 180 and 360
	if(a.y > 0 && b.y < 0) return -1;
	if(a.y < 0 && b.y > 0) return 1;
	//return negative value if cp larger than zero
	return Integer.compare(0, Calc.crossProd(a, b));
	}
}
